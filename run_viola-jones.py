...
def haar_line_v(ii, x, y, w, h):
    ...
    mid = rect_sum(ii, x + third, y, x + 2 * third - 1, y + h - 1)
...
def train_adaboost(X, labels, n_rounds):
    ...
    for rnd in range(n_rounds):
        ...
        # calculate Alpha: alpha = 0.5 * ln((1-err)/err)
...
def train_cascade(X, labels, stage_rounds=(3, 8)):
    cascade = []
...
def extract_features(imgs, feature_bank):
    ...
    X = np.zeros((len(imgs), len(feature_bank)))
    for i, im in enumerate(imgs):
        ii = build_integral_image(im)
        X[i] = extract_all_features(ii, feature_bank)
